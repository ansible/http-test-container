...
RUN set -x && \
    apk add --no-cache openssl ca-certificates py-pip openssl-dev python3-dev libffi-dev gcc libstdc++ make musl-dev && \
    ...
    pip install -c /root/constraints.txt wheel && \
    pip install -c /root/constraints.txt gunicorn httpbin && \
    pip uninstall -y wheel && \
    apk del openssl-dev python3-dev libffi-dev gcc libstdc++ make musl-dev py-pip
